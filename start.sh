...
# Ensure polkit local authority dir exists, then disable polkit for xrdp
...
# Create permissive PAM policies for xrdp to allow logins (authorization bypass)
RUN cat >/etc/pam.d/xrdp-sesman <<'EOF'
...
# Create a runtime fix script that can reapply permissive PAM and restart xrdp services
RUN cat >/fix-auth.sh <<'EOF'
#!/bin/bash
set -e
# Recreate permissive PAM rules
...
# Ensure sesman allows root and disables auth
if [ -f /etc/xrdp/sesman.ini ]; then
    sed -i 's/^DisableAuthentication=.*/DisableAuthentication=true/' /etc/xrdp/sesman.ini || true
    sed -i 's/^AllowRootLogin=.*/AllowRootLogin=true/' /etc/xrdp/sesman.ini || true
    sed -i 's/^RootLoginAllowed=.*/RootLoginAllowed=true/' /etc/xrdp/sesman.ini || true
fi

# Restart xrdp services if present
pkill -x xrdp-sesman 2>/dev/null || true
sleep 1
/usr/sbin/xrdp-sesman --nodaemon &
sleep 2
